...
// int sg_trim_offset_wire(offset_segment_list& orig_list, int zero_length, int overlap);
...
int validate_intersection_as_extension(SPAposition& intersect_pos, COEDGE* c, curve* c_geom, int fixed_start);                                                   // 暂有不懂
int extend_coedges_to_intersection(curve_curve_int* iIntersections, COEDGE* c1, COEDGE* c2, curve* c1_geom, curve* c2_geom, curve* master_crv, SPAposition& p);  // 暂未实现
bool check_if_face_is_full_surface(FACE* original_face, SURFACE* original_SUR, surface* offset_geom);                                                            // 暂未实现
void ofst_init();
...
// int is_planar_wire(WIRE* this_wire, SPAposition& centroid, SPAunit_vector& normal, int apply_transf, int use_winding_number);  // 别的模块
...
int coedge_endpoint_localized_offset_curvature_problem(COEDGE* ce, int at_start, double dist, SPAunit_vector& norm);                 // 已检查
int curve_point_offset_curvature_problem(curve& cu, double param, double dist, int rev, SPAunit_vector& norm);                       // 已检查
int make_tvertex(COEDGE* c, int start);                                                                                              // 暂有不懂
...
double find_max_curv_param(curve* cu, SPAinterval& search_int, SPAposition& max_curv_pos);                        // 不懂
...
int check_gap_type(COEDGE* this_orig_coedge, COEDGE* prev_orig_coedge, SPAvector& plane_nv, double offset_dist);  // 已检查
int is_circle_offset_by_radius(curve* geometry, SPAunit_vector& normal, law* dist_law, law* twist_law);           // 已检查
...
int get_circle_offset_radius(curve* geometry, SPAunit_vector& normal, law* dist_law, law* twist_law, double& off_radius);                                                                                       // 暂有不懂
...
void display_gap_after_close(COEDGE* c1, COEDGE* c2, char* type);                                                                                                              // 暂未实现
int sg_extend_coedges(law* dist_law, SPAposition& in_pos, COEDGE* in_coedge, curve* crv_geom, SPAvector& v_dist);                                                              // 暂未实现
...
int do_boolean(BODY* tool_body, BODY* blank_body, BOOL_TYPE type, BODY*& outside_body, BODY*& leftovers, NDBOOL_KEEP ndbool_keep, BODY*& ndbool_result, glue_options* glue_opts, int quick_failure_preferred);  // 暂未实现
