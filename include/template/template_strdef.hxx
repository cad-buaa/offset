...
/*******************************************************************/
// straigA straight line is represented by a point on itdirection. It also has a scale factor for the paramthat the SPAparameter values can be made itThe parametric equation of the line is :
//		        point = root_point + t * param_scawhere t is the SPAparameter.
/*******************************************************************/
#pragma once
// declare header
...
class DECL_KERN straight : public curve {
  public:
     ...
     * parameter scaling.
     */
    straight(SPAposition const& pos, SPAunit_vector const& dir, double scale = 1.0);
    /**
...
#define ALL_CURVE_DERIVATIVES 9999
     ...
     */
    virtual int accurate_derivs(SPAinterval const& cur = SpaAcis::NullObj::get_interval()) const;
     ...
     * transformation.
     */
    virtual SPAbox bound(SPAposition const& pos1, SPAposition const& pos2, SPAtransf const& trans = SPAtransf()) const;
    /**
     * Returns a box enclosing the two given points on the straight line.
     * <br><br>
     * @param inter
     ...
     * transformation.
     */
    virtual SPAbox bound(SPAinterval const& inter, SPAtransf const& trans = SPAtransf()) const;
     ...
     * transformation.
     */
    SPAbox bound(double start, double end, SPAtransf const& t = SPAtransf()) const {
     ...
     * interval.
     */
    virtual BOUNDING_CYLINDER enclosing_cylinder(const SPAinterval& inter = SpaAcis::NullObj::get_interval()) const;
    /**
     ...
     * approx ok?
     */
    virtual void eval(double val, SPAposition& pos, SPAvector& der1 = SpaAcis::NullObj::get_vector(), SPAvector& der2 = SpaAcis::NullObj::get_vector(), logical flag = FALSE, logical approx = FALSE) const;
    /**
     ...
     * approx ok?
     */
    virtual SPAvector eval_curvature(double param, logical repeat = FALSE, logical approx = FALSE) const;
     ...
     * approx ok?
     */
    virtual double eval_deriv_len(double pt, logical repeat = FALSE, logical approx = FALSE) const;
    /**
     ...
     */
    virtual int evaluate(double parameter, SPAposition& pt, SPAvector** vec = NULL, int deriv = 0, evaluate_curve_side location = evaluate_curve_unknown) const;
    /**
     ...
     */
    virtual double length(double start, double end, logical approx_ok = TRUE) const;
    /**
     ...
     */
    friend DECL_KERN straight operator*(straight const&, SPAtransf const&);
    /**
     ...
     */
    virtual double param(SPAposition const& pos, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter()) const;
    /**
     ...
     * box.
     */
    virtual SPAinterval param_range(SPAbox const& box = SpaAcis::NullObj::get_box()) const;
    /**
     ...
     * the guess parameter value.
     */
    virtual SPAvector point_curvature(SPAposition const& pos, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter()) const;
    /**
     ...
     * the guess parameter value.
     */
    virtual SPAunit_vector point_direction(SPAposition const& pos, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter()) const;
     ...
     * weak flag.
     */
    virtual void point_perp(SPAposition const& point, SPAposition& foot, SPAunit_vector& tangent, SPAvector& curvature, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter(), SPAparameter& param_actual = SpaAcis::NullObj::get_parameter(),
     ...
     * weak flag.
     */
    void point_perp(SPAposition const& point, SPAposition& foot, SPAunit_vector& tangent, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter(), SPAparameter& param_actual = SpaAcis::NullObj::get_parameter(), logical f_weak = FALSE) const {
        point_perp(point, foot, tangent, SpaAcis::NullObj::get_vector(), param_guess, param_actual, f_weak);
    }
    /**
     ...
     * weak flag.
     */
    void point_perp(SPAposition const& point, SPAposition& foot, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter(), SPAparameter& param_actual = SpaAcis::NullObj::get_parameter(), logical f_weak = FALSE) const {
        point_perp(point, foot, SpaAcis::NullObj::get_unit_vector(), SpaAcis::NullObj::get_vector(), param_guess, param_actual, f_weak);
    }
    /**
     ...
     * transformation.
     */
    virtual curve_tancone tangent_cone(SPAinterval const& inter, logical result, SPAtransf const& trans = SPAtransf()) const;
    /**
     ...
     * the actual parameter value.
     */
    virtual logical test_point_tol(SPAposition const& pos, double tol = 0, SPAparameter const& param_guess = SpaAcis::NullObj::get_parameter(), SPAparameter& param_actual = SpaAcis::NullObj::get_parameter()) const;
    /**
