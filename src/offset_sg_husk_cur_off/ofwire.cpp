...
int sg_extend_coedges(law* dist_law, SPAposition& in_pos, COEDGE* in_coedge, curve* crv_geom, SPAvector& v_dist) {
    ...
    double ofst_param;
    if(double(SPAresabs) <= curveature_norm) {
...
int comp_arc_center(SPAposition& iPoint1, SPAposition& iPoint2, const double iRadius, SPAposition& iRefPoint, SPAposition& oCenter) {
    ...
    if(v28 && v30) {
        ...
        intersections = int_cur_cur(*v28, *v30, box_cur, double(SPAresabs) * 0.1);
        if(intersections) {
...
void sg_close_with_arc(offset_segment* this_seg, law* dist_law, SPAposition& p, SPAposition& p1, SPAposition& p2, COEDGE* c1, COEDGE* c2, VERTEX* vert) {
    ...
    TVERTEX* vert_start = (TVERTEX*)c1->end();
    TVERTEX* vert_end = (TVERTEX*)c2->start();
    ...
    if(c_geom->periodic()) {
        if(GET_ALGORITHMIC_VERSION() >= AcisVersion(21, 0, 2)) {
            ...
            if(GET_ALGORITHMIC_VERSION() >= AcisVersion(29, 0, 0)) {
                ...
                if(fixed_start) {
                    int v42 = i >> t || fabs(t.operator double() - i.start_pt()) > fabs(i.end_pt() - t.operator double()) || distFromStart > distFromEnd;
                    return v42;
                } else {
                    int v43 = i >> t || fabs(i.end_pt() - t.operator double()) > fabs(t.operator double() - i.start_pt()) || distFromEnd > distFromStart;
                    return v43;
                }
            } else {
                SPAinterval v93(i.start_pt(), i.start_pt() + period);
                reduce_to_principal_param_range(t, v93, period, double(SPAresnor));
                if(fixed_start) {
                    int v44 = (i >> t) || (fabs(t.operator double() - i.start_pt() - period) > fabs(i.end_pt() - t.operator double()));
                    return v44;
                } else {
        ...
        if(fixed_start) {
            return intersect_param > c_interval_old.start_pt() + double(SPAresnor);
        } else {
            return (c_interval_old.end_pt() - double(SPAresnor)) > intersect_param;
        }
    }
    return answer;
}
    ...
    if(iIntersections && c1 && c2) {
        ...
        while(now_ints) {
            ...
            if(min_dist > dist) {
                ...
                if(v53 && now_ints != iIntersections && (*(double*)&min_dist - dist) / dist < 0.01) {
...
int curve_point_offset_curvature_problem(curve& cu, double param, double dist, int rev, SPAunit_vector& norm) {
...
int coedge_endpoint_localized_offset_curvature_problem(COEDGE* ce, int at_start, double dist, SPAunit_vector& norm) {
    if(!ce->edge()) return 0;
...
int make_tvertex(COEDGE* c, int start) {
    ...
    if(check_e_start && (is_TVERTEX(e->start()))) {
        if(SPAresabs > e->start()->get_tolerance()) return NULL;
        if(!c_sense && c->previous() && c->previous() != c || c_sense == 1 && c->next() && c->next() != c) {
            return 1;
        }
    } else if(!check_e_start) {
        if(is_TVERTEX(e->end())) {
            if(SPAresabs > e->end()->get_tolerance()) return NULL;
...
COEDGE* sg_offset_pl_coedge(COEDGE* c, law* dist_law, law* twist_law, SPAunit_vector& n) {
    EDGE* e = c->edge();
    ...
    if(!curve_offset) {
        curve_offset = (intcurve*)offset_geometry(&e->geometry()->equation(), n, local_law, twist_law, curve_range);
    }
    local_law->remove();
    local_law = nullptr;
                ...
                if(e->closed()) {
                    if(!curve_offset->closed()) {
                        if(GET_ALGORITHMIC_VERSION() >= AcisVersion(14, 0, 1)) {
                            ...
                            v291.begin();
                            get_error_mark().buffer_init = 1;
                            intcurve* intCurOffset = curve_offset;
                            ...
                            bs3Copy = bs3_curve_copy(intCurOffset->cur(-1.0, 0));
    ...
    if(curve_offset) curve_entity_offset = make_curve(*curve_offset);  // 不确
...
int check_gap_type(COEDGE* this_orig_coedge, COEDGE* prev_orig_coedge, SPAvector& plane_nv, double offset_dist) {
    int gap_status = 0;
    SPAposition posOrig;
...
int get_circle_offset_radius(curve* geometry, SPAunit_vector& normal, law* dist_law, law* twist_law, double& off_radius) {
    int answer = 0;
...
int is_circle_offset_by_radius(curve* geometry, SPAunit_vector& normal, law* dist_law, law* twist_law) {
    int answer = 0;
...
int comp_tangent_at_coedge_commom_vertex(COEDGE* iCoedge1, COEDGE* iCoedge2, COEDGE* iOfstCoedge1, COEDGE* iOfstCoedge2, SPAunit_vector& oTangent, SPAunit_vector& oStartOfstDir, SPAunit_vector& oEndfstDir) {
    ...
    int retOK;
    if(antiparallel(tan1, tan2, double(SPAresnor))) {
...
int get_coedge_data(COEDGE* iCoedge, VERTEX*& oVertex, SPAposition& oVrtPos, curve*& oCurGeom, int iNeedStartData) {
    ...
    if(retOk) {
        ...
        if(oVertex && oVertex->geometry()) {
            oVrtPos = oVertex->geometry()->coords(), sizeof(SPAposition);
        } else {
            retOk = 0;
        }
    }
        ...
        if(ioCoedge->edge()) {
            if(ioSplitVertex) {
                if(ioCurve) {
                    ...
                    if(iParama.operator double() > started) {
            ...
            if(coedge_start_dir(ioCoedge2) % gapVec < 0.0) {
                ...
                if((ioCoedge1->start_pos() - p1).len_sq() > gapLen2) {
                    if((ioCoedge2->end_pos() - p2).len_sq() > gapLen2) {
                        ...
                        cur1->point_perp(p2, newPos1, param_guess, newParam1, 0);
                        SPAparameter parameter = SpaAcis::NullObj::get_parameter();
                        cur2->point_perp(p1, newPos2, parameter, newParam2, 0);
                        ...
                        if(overDst2 > (p2 - newPos1).len()) {
                            if(overDst2 > (p1 - newPos2).len()) {
                                if((p1 - p2).len() > overDst2) {
                                    ...
                                    if(trim_coedge(ioCoedge1, vrt1, newPos1, newParam1, cur1, v27)) {
...
int sg_close_offset_gap(offset_segment_list& seg_list, offset_segment*& this_seg, COEDGE* this_coedge, VERTEX* common_vertex, SPAunit_vector& iOffsetDir, SPAunit_vector& iStartOfstDir, SPAunit_vector& iEndOfstDir, law* dist_law, sg_gap_type gap_type,
    ...
    int close_gap = v61;
    if(v61 && SPAresfit > gap_size) {
        if(2.0 * SPAresabs > gap_size) {
    ...
    if(!close_gap && !skipbackup && vert1 != vert2) make_same_vertex_pointers(vert1, vert2, this_coedgea, next_coedge, 0);
    return close_gap;
}
...
BODY* sg_offset_planar_wire_internal(WIRE* wire, TRANSFORM* wire_transf, law* dist_law, law* twist_law, SPAunit_vector& wire_normal, sg_gap_type close_type, int add_attribs, int trim, int zero_length, int overlap, int iKeepMiniTopo,
    ...
    int dist_law_size;
    law** dist_laws = v409->map_laws(dist_law, &dist_law_size, 0);
    ...
    COEDGE* prev_coedgea;
    do {
        COEDGE* offset_coedge = sg_offset_pl_coedge(this_coedge, dist_laws[coedge_count], twist_laws[coedge_count], wire_normal);  // 有待探究
        // 空几何，WIRE开放,原曲线非圆
        if(offset_coedge) {
    ...
    if(!off_coedge_list[0] || off_coedge_list.count() > 1 && all_offset_degenerated(off_coedge_list))  // 所有的coedge的edge都没有CURVE
    {
        ...
        twist_laws = nullptr;
        if(resignal_no || acis_interrupted()) sys_error(resignal_no, error_info_base_ptr);
        BODY* v439 = ACIS_NEW BODY(offset_wire);
        BODY* offset_wire_body = v439;
        TRANSFORM* v441 = nullptr;
        if(wire_transf) {
            v441 = ACIS_NEW TRANSFORM((SPAtransf)wire_transf->transform());
        }
            ...
            while(this_seg != last_seg) {
                ...
                if(is_intcurve_edge(ofc->edge())) {
                    if(is_intcurve_edge(pgc->edge())) {
                        intcurve& ofintc = (intcurve&)ofc->edge()->geometry()->equation();
                        intcurve& pgintc = (intcurve&)pgc->edge()->geometry()->equation();
                        long double t1 = bs3_curve_angle(pgintc.cur(-1.0, 0));
                        long double t2 = bs3_curve_angle(ofintc.cur(-1.0, 0));
                        if(t2 > t1 + 3.0) {
                            ...
                            v663.begin();
                            get_error_mark().buffer_init = 1;
                            new_bs3 = bs3_curve_make_approx(ofintc_upd, the_int, ofintc.fitol(), actual_tol, 0);
                            if(new_bs3) {
                                long double rem_tol = actual_tol;
                                if(double(SPAresabs) > rem_tol) rem_tol = double(SPAresabs);
                                bs3_curve_rem_extra_knots(new_bs3, rem_tol);
                                long double t3 = bs3_curve_angle(new_bs3);
                                if(t1 + 3.0 > t3) {
            ...
            for(EDGE* edge = first_edge; edge; edge = (EDGE*)edges.next()) {
                if(edge->geometry()) {
                    ...
                    if(((curve&)edge_cur).type() == 11) {
                        ...
                        _deriv_ptr[0] = &dpdt;
                        _deriv_ptr[1] = &dp2dt2;
                        long double t_val = edge->start_param().operator double();
                        ...
                        if(edge->start_param() < edge->end_param())
                            ...
                            dir = -1;
                        edge_cur.evaluate(dir, pt, _deriv_ptr, 2, evaluate_curve_unknown);
                        ...
                        long double start_par = edge->start_param().operator double();
                        long double end_par = edge->end_param().operator double();
                        ...
                        if(start_curvature.len() > 1.0 / SPAresabs) {
                            ...
                            if(start_t_step == max_t_step) {
                                ...
                                while(1) {
                                    edge_cur.evaluate(t_adjust, pt, _deriv_ptr, 2);  // 不确
                                ...
                                while(1) {
                                    ...
                                    if(conv_curvature(*_deriv_ptr[0], *_deriv_ptr[1]).len() <= 1.0 / SPAresabs) {
                                        ...
                                        if(double(SPAresmch) > t - v368) break;
                                        v368 = t;
                                    }
                                }
                        ...
                        if(end_curvature.len() > double(SPAresabs)) {
                            ...
                            if(end_t_step == max_t_step) {
                                ...
                                while(1) {
                                    edge_cur.evaluate(v350, pt, _deriv_ptr, 2);  // 不确
                                    if(conv_curvature(*_deriv_ptr[0], *_deriv_ptr[1]).len() > double(SPAresabs)) break;
                                    if(same_point(v668, pt, double(SPAresabs))) end_t_step = v350 / v311;
                                    v311 = v311 * 2.0;
                                    v668 = pt;
                                }
                                ...
                                while(1) {
                                    if(same_point(v668, pt, double(SPAresabs))) break;
                                    double v365 = (v391 + v364) / 2.0;
                                    edge_cur.evaluate(v365, pt, _deriv_ptr, 2);  // 不懂包括上面类似的句子
                                    if(conv_curvature(*_deriv_ptr[0], *_deriv_ptr[1]).len() <= 1.0 / SPAresabs)
                                        v364 = v365;
                                    else
                                        v391 = v365;
...
BODY* sg_offset_planar_wire(BODY* wire_body, double offset_dist, SPAunit_vector& wire_normal, sg_gap_type close_type, int add_attribs, int trim, int zero_length, int overlap) {
    ...
    if(wire_body) {
        ...
        // ndbool_result = SpaAcis::NullObj::get_body_ptr();
        // leftovers = SpaAcis::NullObj::get_body_ptr();
        // body_ptr = SpaAcis::NullObj::get_body_ptr();
        // do_boolean(this_body, result_body, UNION, body_ptr, leftovers, NDBOOL_KEEP_NEITHER, ndbool_result, 0i64, 0);//
        // }
        // else
        // {
        ...
        for(LUMP* lump = wire_body->lump(); lump; lump = lump->next(PAT_CAN_CREATE)) {
            for(SHELL* shell = lump->shell(); shell; shell = shell->next(PAT_CAN_CREATE)) {
                for(WIRE* i = shell->wire(); i; i = i->next(PAT_CAN_CREATE)) {
                    ...
                    if(result_body) {
                        do_boolean(tool_body, result_body, UNION, SpaAcis::NullObj::get_body_ptr(), SpaAcis::NullObj::get_body_ptr(), NDBOOL_KEEP_NEITHER, SpaAcis::NullObj::get_body_ptr(), nullptr, 0);  // 不确定
                    } else {
